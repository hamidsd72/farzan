...
@section('body')
    ...
    <div class="login">
        <header class="header-login pull-right w-100">
            <h1 class="pull-right w-100 text-center text-white">ورود به سایت</h1>
            <p class="pull-right w-100 mt-2 text-center text-white">ورود به پروفایل کاربری</p>
            @if(!is_null(session()->get('errors')))
                <a href="#" class="pull-right w-100 mt-2 text-center text-white"
                   style="font-size: 15px;color: red !important;">
                    شماره موبایل یا رمز عبور اشتباه است (در صورت فراموشی رمز عبور
                    <span style="color: white" style="cursor: pointer" data-toggle="modal" data-target="#exampleModal">اینجا کلیک کنید )
                        </span>
            ...
            <form action="{{ route('login') }}" method="POST" class="form-horizontal pull-right w-100">
                <fieldset>
                    ...
                    <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                @if ($errors->has('email'))
                                    <p class="text-center"
                                       style="padding: 8px;background-color: #ff000094;color: white; margin-top: -62px;">
                                        <strong>{{ $errors->first('email') == 'auth.failed' ? 'نام کاربری یا رمز عبور اشتباه است' : $errors->first('email') }}
                                        </strong>
                                    </p>
                                @endif
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <input type="text" class="form-control" id="email" name="mobile"
                                       placeholder="نام کاربری(شماره موبایل)" value="{{ old('email') }}"/>
                    ...
                    <div class="form-group{{ $errors->has('password') ? ' has-error' : '' }}">
                        <div class="row">
                            <div class="col-md-12">
                                <a style="cursor: pointer" data-toggle="modal" data-target="#exampleModal">رمز عبور
                                    را
                                    فراموش کردید ؟ </a>
                            </div>
                        </div>
